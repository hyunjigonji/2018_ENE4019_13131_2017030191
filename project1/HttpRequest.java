...
enum StatusCode{
	OK, BAD_REQUEST, FORBIDDEN, NOT_FOUND, HTTP_VERSION_NOT_SUPPORTED, INTERNATIONAL_SERVER_ERROR,
}
...
final class HttpRequest implements Runnable{
	...
	private String parseRequestLine(String requestLine) {
		...
		if(!file.canRead()) {
			...
			return "Requested file " + fileName + " is not readable. " + "Returning with FORBBIDEN.";
		}
		if(file.isDirectory()) {
			File[] list = file.listFiles(new FilenameFilter(){
				public boolean accept(File dir, String f) {
					if(f.equalsIgnoreCase("index.html")) return true;
					return false;
				}
			});
			if(list == null || list.length == 0) {
				...
				code = StatusCode.INTERNATIONAL_SERVER_ERROR;	
				return "Found more than one index file at requested location " + fileName + ". Returning INTERNATIONAL SERVER ERROR.";
			}
			file = list[0];
		}
		requestedFile = file;
	...
	private void sendResponseMessage(FileInputStream fis, DataOutputStream os) throws Exception{
		...
		if(code != StatusCode.OK) {
			entityBody = entityBody.replaceAll("\\?", message + " - sent by Hyunji's WebServer");
		}
	...
	private void sendHeaderLines(DataOutputStream os) throws Exception{
		...
		case OK:
			...
			contentTypeLine = contentTypeLine + "content-length: " + 1024 + CRLF;
			break;
		default:
			contentTypeLine += "text/html" + CRLF;
		}
		//headerLines.append((contentTypeLine + CRLF));
		os.writeBytes(contentTypeLine.toString());
	}
